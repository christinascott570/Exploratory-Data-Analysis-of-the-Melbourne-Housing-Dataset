...
begin
	#New data frame for columns and their missing values
	dfMissing = DataFrame(Column = String[], MissingValues = Int[])
		#Loop through each column of original dataframe
		for col in names(df)
			missing_count = count(ismissing, df[!, col])
			#Add a new row when missing content is found
...
# ╔═╡ b9354816-86a4-499e-b73e-d197e405f32a
begin
	dfPlot = @chain df begin
		@select(Regionname, Price, Rooms, Latitude, Longtitude, Distance, Type)
		@filter(Regionname in ["Northern Metropolitan", "Western Metropolitan", "Southern Metropolitan", "Eastern Metropolitan", "South-Eastern Metropolitan"])
		@drop_missing()
		#Create new columns to replace old
		@mutate(
			Region = replace(Regionname,
			...
			PriceK = Price/1000,
			TypeFull= replace(Type,
			"h" => "House",
			"t" => "Townhouse",
			"u" => "Unit"),
			Colour = replace(Regionname,
			"Northern Metropolitan" => 1,
			"Western Metropolitan" => 2,
			"Southern Metropolitan" => 3,
			"Eastern Metropolitan" => 4,
			"South-Eastern Metropolitan" => 5))
	#Select new columns for output
	@select(Region, PriceK, TypeFull, Colour, Rooms, Latitude, Longtitude, Distance)
	end
end
...
let
	...
	scatter(points, alpha=0.1,
		color = colors[parse.(Int, dfPlot.Colour)])
end
...
# ╔═╡ 8f150b0e-b8ab-4eb0-8fc0-c33c70f8540e
begin
	#Calculate average
	avgPrice = mean(dfPlot.PriceK)
	#Create new subsets for those above average and below
	aboveAvg = dfPlot[dfPlot.PriceK .> avgPrice, :]
	belowAvg = dfPlot[dfPlot.PriceK .<= avgPrice, :]

	#Create points for subsets by their longtitude and latitude
	points_above = Point2f.(aboveAvg.Longtitude, aboveAvg.Latitude)
	points_below = Point2f.(belowAvg.Longtitude, belowAvg.Latitude)

let 
	fig = Figure()
	ax = Axis(fig[1, 1], title="Geographical Distribution of Above and Below Average Priced Houses")

	#Plot points for those below 
	scatter!(ax, points_below, label="Below Average Price", color=:blue, alpha=0.2)
	#Plot points for those above
	scatter!(ax, points_above, label="Above Average Price", color=:yellow, alpha=0.155)
...
# ╔═╡ 6ee69c7a-c0c6-4dfd-bb2c-a188545ba546
md"""
...
# ╔═╡ 9c12d5ed-4c47-4680-8af9-1d57c13c84cc
md"""
...
# ╔═╡ bc1dabcb-1d63-4883-a02c-1f8d0314a021
begin
southern_metro_data = dfPlot[dfPlot.Region .== "SEM", :]
	housing_distribution = @chain southern_metro_data begin
    @group_by(TypeFull)
    @summarize(
        Count = n(),
        AveragePriceK = mean(PriceK)
    )
end
end 

# ╔═╡ ba79f5d6-b2c3-46bb-ae2b-c62cbf17715f
md"""
From this updated dataset we can see that units cost significantly less than houses and townhouses.
"""

# ╔═╡ 96257ed2-288d-49cb-819e-dbb137d71c94
md"""
...
# ╔═╡ a5d5da6e-40dd-4677-8813-da4374aaa2c0
let
    xs = dfPlot.Longtitude
    ys = dfPlot.Latitude
    points = Point2f.(xs, ys)

    houses = dfPlot[dfPlot.TypeFull .== "House", :]
    townhouses = dfPlot[dfPlot.TypeFull .== "Townhouse", :]
    units = dfPlot[dfPlot.TypeFull .== "Unit", :]

    points_houses = Point2f.(houses.Longtitude, houses.Latitude)
    points_townhouses = Point2f.(townhouses.Longtitude, townhouses.Latitude)
    points_units = Point2f.(units.Longtitude, units.Latitude)


    fig = Figure()
    ax = Axis(fig[1, 1], title="Geographical Distribution of House Types in Melbourne")

    scatter!(ax, points_houses, label="Houses", color=:blue, alpha=0.1)
    scatter!(ax, points_townhouses, label="Townhouses", color=:green, alpha=0.1)
    scatter!(ax, points_units, label="Units", color=:red, alpha=0.1)
...
# ╔═╡ b3000bb4-030b-4911-817f-bf2171ca507a
md"""
...
begin
    distPrice = @chain dfPlot begin
		#return nearest integral value
        @mutate(Dist = trunc.(Distance))  
        @mutate(Dist = Int.(Dist))        
        @group_by(Dist)                   
        @summarize(
            n = n(),                       
            AvgPrice = mean(PriceK)         
        )
    end
end
...
# ╔═╡ 141e1b93-c8ff-4999-b747-4e5eb2a41434
@bind poly_order PlutoUI.Slider(1:1:5, default=3)

# ╔═╡ ad50bb04-6265-4a38-9c2b-cfb6924aa6e9

let
    xs = distPrice.Dist
    ys = distPrice.AvgPrice

    function fit_polynomial(xs, ys, degree)
		#Creating the fitted line
        p = Polynomials.fit(xs, ys, degree)
		#Predict values using fitted polynomial
        ys_pred = p.(xs)

        # Calculate R², RMSE, and correlation
        R2 = 1 - sum((ys .- ys_pred).^2) / sum((ys .- mean(ys)).^2)
        rmse = sqrt(mean((ys .- ys_pred).^2))
        correlation = cor(xs, ys)
		...
		#Original data points (distance & price)
        scatter!(ax, xs, ys, color=:blue)

        # Sort xs and ys_pred for proper line plotting
        sorted_indices = sortperm(xs)
        sorted_xs = xs[sorted_indices]
        sorted_ys_pred = ys_pred[sorted_indices]

		#Plot fitted line
        lines!(ax, sorted_xs, sorted_ys_pred, color=:red, linewidth=3)

        # Set custom y-ticks
        y_ticks = 600:200:1400  
        ax.yticks = y_ticks
        
        text!(ax, 25, 1400, text="Correlation: $(round(correlation, digits=3))", fontsize=16)
        text!(ax, 25, 1300, text="R²: $(round(R2, digits=3))", fontsize=16)
        text!(ax, 25, 1200, text="RMSE: $(round(rmse, digits=1))", fontsize=16)

        return fig
    end

    fit_polynomial(xs, ys, poly_order) #function call
end

# ╔═╡ b6033820-66cb-46c0-ae56-797fe04dd877
@test round(cor(distPrice.Dist, distPrice.AvgPrice),digits=3) == -0.725

# ╔═╡ 1aee4b17-2416-4555-81f1-f6cc3c10c29f
md"""

The degree 3 polynomial best captures the underlying relationship between house prices and distance from the CBD. While the degree 5 polynomial provides the best statistical fit—reflected by a higher R² and lower RMSE—it runs the risk of overfitting. Overfitting occurs when a model is too finely tuned to the specific dataset it was trained on, leading to poor performance on new data.
...
manifest_format = "2.0"
project_hash = "f53deefc4db9d669f4a70ecddcf7fb62246f80bd"
